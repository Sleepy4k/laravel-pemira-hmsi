document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        sidebarLinks.length && sidebarLinks.forEach(link => {
            ...
            if (typeof Swal !== "undefined") {
                window.Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (logoutForm) {
                            const logoutAction = logoutForm.getAttribute('data-action') || '';
                            if (logoutAction) {
                                window.axios.delete(logoutAction).then(response => {
                                    if (response.data && response.data.status === 'success') {
                                        const redirectUrl = logoutForm.getAttribute('data-redirect') || '/';
    ...
    if (dropdown && dropdownButton && dropdownArrow) {
        dropdownButton.addEventListener('click', function(e) {
            ...
            dropdownArrow.style.transform = dropdown.classList.contains('hidden') ? 'rotate(0deg)' : 'rotate(180deg)';
        });
